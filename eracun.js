...
streznik.get('/', function(zahteva, odgovor) {
  // Če ni izbrane stranke preusmeri na '/prijava'
  if(!zahteva.session.stranka) return odgovor.redirect('/prijava');
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    strankaIzRacuna(racunId, function(stranke) {
      var narocnikRacuna = stranke[0];
      //console.log(narocnikRacuna);
      pesmiIzRacuna(racunId, function(pesmi) {
        odgovor.setHeader('content-type', 'text/xml');
        odgovor.render('eslog', {
          vizualiziraj: true,
          postavkeRacuna: pesmi,
          narocnikRacuna: narocnikRacuna
        })
      })
    })
  })
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    if (!pesmi) {
      ...
      // Izpis racuna za prijavljeno stranko
      var stranka = zahteva.session.stranka;
      vrniStranko(stranka, function(vrstice) {
        var narocnikRacuna = vrstice[0];
        //console.log(narocnikRacuna);
        odgovor.setHeader('content-type', 'text/xml');
        odgovor.render('eslog', {
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    //Prikaz strani za prijavo s sporočilom o uspeli registraciji
    var sporocilo = "Stranka je bila uspešno registrirana.";
    vrniStranke(function(napaka3, stranke) {
      vrniRacune(function(napaka4, racuni) {
        if(napaka1 || napaka2 || napaka3 || napaka4) sporocilo = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.stranka = polja.seznamStrank; // Zaporedna številka izbrane stranke
...
streznik.post('/odjava', function(zahteva, odgovor) {
    delete zahteva.session.stranka;
